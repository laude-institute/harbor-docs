---
title: Metrics
description: Defining dataset-level metrics for reward aggregation
---
...
Currently, custom metrics are only supported locally. Please [raise a GitHub issue](https://github.com/laude-institute/harbor/issues) if you want support for hosted custom metrics.
...
where `input.jsonl` is the trial rewards and `output.json` will contain your computed metric(s). If your script requires dependencies, add them as a comment [using uv's convention](https://docs.astral.sh/uv/guides/scripts/#declaring-script-dependencies).

Here is an example of a custom metric script:

```python title="custom_mean.py"
# /// script
# dependencies = [
#   "numpy==2.3.4",
# ]
