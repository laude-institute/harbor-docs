...
Mini-SWE-Agent is installed from [PyPI](https://pypi.org/project/mini-swe-agent/) as the `mini-swe-agent` package. Harbor handles the installation automatically when you use the `--agent mini-swe-agent` flag.
...
Harbor installs Mini-SWE-Agent directly from [PyPI](https://pypi.org/project/mini-swe-agent/) (`uv tool install mini-swe-agent`), **not** from a git repository. The CLI provides two equivalent entry points: `mini` and `mini-swe-agent`.
</Callout>
