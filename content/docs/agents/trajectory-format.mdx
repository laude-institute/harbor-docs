...
- **[Mini-SWE-Agent](/docs/agents/mini-swe-agent)** - Software engineering agent with trajectory support (converts `mini-swe-agent-1.1` format to ATIF)
...
[Mini-SWE-Agent](/docs/agents/mini-swe-agent) v2 produces trajectories in its native `mini-swe-agent-1.1` format, which Harbor converts to ATIF. The v2 format uses tool-calling (messages with `"role": "tool"` and `tool_call_id`) rather than the v1 text-based format (which used `"role": "user"` for observations and parsed ```` ```bash ```` blocks from assistant messages):

```python
# From harbor/agents/installed/mini_swe_agent.py
def populate_context_post_run(self, context: AgentContext) -> None:
    """Convert Mini-SWE-Agent v2 trajectory to ATIF format."""
    ...
    # Extract token usage from v2 format
    # v2 stores cost in extra.cost per assistant message
    # and actions in extra.actions (not parsed from text)
    for message in mini_trajectory.get("messages") or []:
        role = message.get("role")
        extra = message.get("extra") or {}

        if role == "assistant":
            # v2: actions are structured, not parsed from bash blocks
            actions = extra.get("actions", [])
            step_cost = extra.get("cost", 0)

        elif role == "tool":
            # v2: observations are "tool" messages with JSON content
            tool_call_id = message.get("tool_call_id")
            content = json.loads(message.get("content", "{}"))
            returncode = content.get("returncode")
            output = content.get("output")

    # Convert to ATIF and save
...
The conversion maps Mini-SWE-Agent v2 messages to ATIF steps: `system` and `user` messages become system/user steps, `assistant` messages become agent steps with structured `tool_calls` from `extra.actions`, `tool` messages become `observation` results on the preceding agent step, and `exit` messages capture the final submission status.
